...
このフォークの調査・実装・ドキュメントは [**Claude Code**](https://claude.com/claude-code)（Anthropic）を使って行いました。
Claude Code は、ターミナル上で動作する Anthropic 公式の AI コーディングエージェントです。コードベースの調査、
バグ修正、リファクタリング、Git 操作までを自然言語で任せられます。本フォークの「マウストラッキング中の選択対応」や
「256色化」も、TeraTerm の C/C++ コードを Claude Code に調査させて原因を特定し、修正まで仕上げたものです。

- 公式サイト: https://claude.com/claude-code
- 最新モデル（Claude Opus 4.8 など）を搭載し、ターミナル / VS Code / JetBrains などで利用できます。
